...
def build(opt: BuildOptions):
    ...
    platforms = [
        ('cp', 'manylinux_x86_64', opt.manylinux_images['x86_64']),
        ('cp', 'manylinux_i686', opt.manylinux_images['i686']),
        ('cp', 'manylinux_aarch64', opt.manylinux_images['aarch64']),
        ('cp', 'manylinux_ppc64le', opt.manylinux_images['ppc64le']),
        ('cp', 'manylinux_s390x', opt.manylinux_images['s390x']),
        ('pp', 'manylinux_x86_64', opt.manylinux_images['pypy_x86_64']),
    ]
    ...
    for implementation, platform_tag, docker_image in platforms:
        ...
        try:
            ...
            call(['docker', 'cp',
                  os.path.abspath(opt.project_dir) + '/.',
            ...
            for config in platform_configs:
                if opt.dependency_constraints:
                    constraints_file = opt.dependency_constraints.get_for_python_version(config.version)
                ...
                call(
                    ...
                    '''.format(
                        ...
                        test_requires=' '.join(opt.test_requires),
                        test_extras=opt.test_extras,
                        test_command=shlex.quote(
                            prepare_command(opt.test_command, project='/project') if opt.test_command else ''
                        ),
                        before_build=shlex.quote(
                            prepare_command(opt.before_build, project='/project') if opt.before_build else ''
                        ),
                        build_verbosity_flag=' '.join(get_build_verbosity_extra_flags(opt.build_verbosity)),
                        repair_command=shlex.quote(
                            prepare_command(opt.repair_command, wheel='"$1"', dest_dir='/tmp/repaired_wheels') if opt.repair_command else ''
                        ),
                        environment_exports='\n'.join(opt.environment.as_shell_commands()),
                        uid=os.getuid(),
                        gid=os.getgid(),
                        before_test=shlex.quote(
                            prepare_command(opt.before_test, project='/project') if opt.before_test else ''
                        ),
                        dependency_install_flags='-c /constraints.txt' if opt.dependency_constraints else '',
                    )
                )
            ...
            call(['docker', 'cp',
                  ...
                  os.path.abspath(opt.output_dir)])
