...
If you need to specify different compilation flags or other properties on a per-ABI or per-CPU basis, you can use [configuration overrides](..#overrides) with a `select` clause that targets the specific ABI or architecture. For example, consider the following example:
 ...
 * Add `arm64-testing-helper` to the test environment for all ARM64 devices (whether simulator or device).
