...
def test_build_default_launches(mock_build_docker, fake_package_dir, monkeypatch):
    ...
    assert "quay.io/pypa/manylinux2014_x86_64" in kwargs["docker"]["image"]
    ...
    assert "quay.io/pypa/manylinux2014_i686" in kwargs["docker"]["image"]
    assert kwargs["docker"]["cwd"] == PurePosixPath("/project")
    ...
    assert "quay.io/pypa/musllinux_1_1_x86_64" in kwargs["docker"]["image"]
    assert kwargs["docker"]["cwd"] == PurePosixPath("/project")
    assert not kwargs["docker"]["simulate_32_bit"]
    ...
    assert "quay.io/pypa/musllinux_1_1_i686" in kwargs["docker"]["image"]
    assert kwargs["docker"]["cwd"] == PurePosixPath("/project")
    assert kwargs["docker"]["simulate_32_bit"]
...
"""
    ...
    kwargs = build_on_docker.call_args_list[2][1]
    assert "quay.io/pypa/manylinux_2_24_x86_64" in kwargs["docker"]["image"]
    assert kwargs["docker"]["cwd"] == PurePosixPath("/project")
    assert not kwargs["docker"]["simulate_32_bit"]
    identifiers = {x.identifier for x in kwargs["platform_configs"]}
    assert identifiers == {
